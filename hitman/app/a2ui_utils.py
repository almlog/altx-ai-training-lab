...
_FALLBACK_TEXT = (
    "申し訳ありません。手順書のUI描画で問題が発生しました。もう一度お試しいただくか、確認したいステップ番号をお知らせください。"
)
...
def _extract_prose_text(text: str) -> str:
    """Extract natural language text from model output that also contains A2UI JSON."""
    clean = re.sub(r"```(?:json|a2ui)?[\s\S]*?```", "", text)
    clean = re.sub(r"<a2ui-json>[\s\S]*?</a2ui-json>", "", clean, flags=re.I)
    clean = re.sub(r"<a2a_datapart_json>[\s\S]*?</a2a_datapart_json>", "", clean, flags=re.I)
    prose_lines = []
    in_raw_json = False
    for line in clean.splitlines():
        ...
        if stripped.startswith("{") or stripped.startswith("["):
            in_raw_json = True
        if not in_raw_json and stripped:
            prose_lines.append(line)
        if in_raw_json and (stripped.endswith("}") or stripped.endswith("]")):
            in_raw_json = False
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        # Auto-inject beginRendering if missing
        has_begin_rendering = any("beginRendering" in m for m in messages)
        if not has_begin_rendering:
            for m in messages:
                su = m.get("surfaceUpdate")
                if isinstance(su, dict) and su.get("components"):
                    first_comp_id = su["components"][0].get("id", "root")
                    surface_id = su.get("surfaceId", "default")
                    messages.insert(0, {"beginRendering": {"surfaceId": surface_id, "root": first_comp_id}})
                    break

        prose = _extract_prose_text(text)

        if not _surface_is_renderable(messages):
            fallback = prose if prose else _FALLBACK_TEXT
            return LlmResponse(
        ...
        # In ADK Web, the frontend natively parses <a2ui-json> blocks embedded in the text
        # via extractA2uiJsonFromText(s). It populates a2uiData and strips the tag from the chat bubble.
        a2ui_json_str = json.dumps(messages, ensure_ascii=False)
        combined_text = (
            f"{prose}\n\n<a2ui-json>\n{a2ui_json_str}\n</a2ui-json>"
            if prose
            else f"<a2ui-json>\n{a2ui_json_str}\n</a2ui-json>"
